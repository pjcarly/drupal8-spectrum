...
class ModelApiHandler extends BaseApiHandler
{
  ...
  /**
   * Indicate whether the GET response should only return a single result instead of a list
   *
   * @return boolean
   */
  protected function shouldReturnSingle(): bool
  {
    return FALSE;
  }
    ...
    if (empty($this->slug)) {
      ...
      if ($request->query->has('filter')) {
        ...
        if (is_array($filter)) {
          ...
          $listview = $this->modelApiService->getListViewForFilterArray($modelClassName, $filter);
          if (!empty($listview)) {
      ...
      $returnSingle = $request->query->has('_single') || $this->shouldReturnSingle();
      if ($returnSingle) {
        $query->setLimit(1);
      ...
      if ($hasResults) {
        ...
        if (!$onlyIds) {
          ...
          $data = null;
          if ($node instanceof JsonApiDataNode) {
            $data = $node;
          } else {
